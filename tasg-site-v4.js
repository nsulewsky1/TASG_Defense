...
// SNARE-1A interactive concept iframe sizing
(function () {
  const frame = document.getElementById('snareConceptFrame');
  if (!frame) return;
  window.addEventListener('message', (event) => {
    if (event.origin !== window.location.origin && event.origin !== 'null') return;
    const data = event.data;
    if (!data || data.type !== 'tasg-snare-concept-height') return;
    const height = Number(data.height);
    if (!Number.isFinite(height)) return;
    frame.style.height = `${Math.max(760, Math.min(3200, Math.ceil(height + 4)))}px`;
  });
})();
